...
% MPC-425, Exercise 2
%%%
function QuadraticOptimization
...
while kappa > opt.epsilon % Stop once the barrier term is small
  ...
  while 1
    ...
    % Compute search Newton's direction 
    summ = 0;
    summ2 = 0;
    for k = 1:length(prob.d)
        di = prob.d(k);
        gi = prob.G(k,:);
        summ = summ + 1/(di-gi*z).^2*gi'*gi;
        summ2 = summ2 + 1/(di-gi*z)*gi';
    end
    A = prob.H + kappa*summ;
    b = -prob.H*z - prob.q - kappa*summ2;
